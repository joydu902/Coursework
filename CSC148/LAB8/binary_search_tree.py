...
def find_max(t: BinarySearchTree) -> bool:
    """
...
def delete(t: BinarySearchTree, value: Any) -> bool:
    """
    ...
    if value < t.value:
        ...
        # If there are no children left; just return None (deleting a leaf)
        if not t.left and not t.right:
            return None
        
        # The values are the same. If one child is missing, then the other child
        # can be the new root.
        ...
        # If both children exist, swap the root with the largest in the left
        # subtree, and delete that item from the left subtree.
        max_value = find_max(t.left)
        t.value = max_value
        t.left = delete(t.left, max_value)
